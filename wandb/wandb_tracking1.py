...
from src.models.train_model1 import train_and_evaluate
...
def main():
    ...
    config = {
        ...
        "sampling": {
            "method": "smote",          
            "enabled": True,            
            "sampling_strategy": 0.2,   # minoritaire = 20% de la majorité
            "k_neighbors": 5
        }

    }

    if config.get("sampling", {}).get("enabled") and config.get("sampling", {}).get("method") == "smote":
        if not IMBLEARN_AVAILABLE:
            print("WARNING: imblearn non installé -> SMOTE désactivé. pip install imbalanced-learn")
            config["sampling"]["enabled"] = False

    if config.get("sampling", {}).get("enabled") and config.get("model_type") in ("xgboost", "lightgbm"):
        print("NOTE: SMOTE + XGBoost/LightGBM fonctionne, mais parfois on préfère scale_pos_weight sans SMOTE.")
...
#ajout smote
        ...
        if target_col in train_df.columns:
            vc_train = train_df[target_col].value_counts(dropna=False).to_dict()
            experiment.log_metrics({
                "class_0_train": int(vc_train.get(0, 0)),
                "class_1_train": int(vc_train.get(1, 0)),
            })
        if test_df is not None and target_col in test_df.columns:
            vc_test = test_df[target_col].value_counts(dropna=False).to_dict()
            experiment.log_metrics({
                "class_0_test": int(vc_test.get(0, 0)),
                "class_1_test": int(vc_test.get(1, 0)),
            })
        ...
        use_smote = bool(sampling_cfg.get("enabled") and sampling_cfg.get("method") == "smote")
        smote_sampling_strategy = sampling_cfg.get("sampling_strategy", 0.2)
        smote_k_neighbors = sampling_cfg.get("k_neighbors", 5)
        ...
        # smote actif ou non
        experiment.log_metrics({
            "smote_enabled": int(use_smote),
            "smote_sampling_strategy": float(smote_sampling_strategy) if use_smote else 0.0,
            "smote_k_neighbors": int(smote_k_neighbors) if use_smote else 0
        })

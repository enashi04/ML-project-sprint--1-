...
def add_failure_indicators(sensor_df, failure_df, time_window=24):
    ...
    for _, failure in failure_df.iterrows():
        ...
        if window_mask.any():
            idx_window = sensor_df.index[window_mask]

            ttf_hours = (failure_time - sensor_df.loc[idx_window, "timestamp"]).dt.total_seconds() / 3600

            current_ttf = pd.to_numeric(sensor_df.loc[idx_window, "time_to_failure"], errors="coerce")
            update_mask = current_ttf.isna() | (ttf_hours < current_ttf)

            idx_to_update = idx_window[update_mask.to_numpy()]

            sensor_df.loc[idx_to_update, "failure_soon"] = 1
            sensor_df.loc[idx_to_update, "time_to_failure"] = ttf_hours.loc[idx_to_update].to_numpy()
            sensor_df.loc[idx_to_update, "next_failure_type"] = failure_type

    print(sensor_df["failure_soon"].value_counts(dropna=False))
    ...
    for equipment_id in sensor_df['equipment_id'].unique():
        ...
        for idx, sensor_row in equip_sensors.iterrows():
            ...
            if len(prev_failures) > 0:
                ...
                sensor_df.loc[idx, 'days_since_last_failure'] = (current_time - last_failure_time).total_seconds() / (3600 * 24)

                # Compter les défaillances dans les 30 derniers jours
                recent_failures = prev_failures[prev_failures['failure_timestamp'] >= current_time - pd.Timedelta(days=30)]
                sensor_df.loc[idx, 'failures_count_last_30days'] = len(recent_failures)
...
def plot_feature_importances(df, target_col='failure_soon', output_path=None):
    """
    Trace et sauvegarde les corrélations entre les caractéristiques et la cible.
    (Version robuste : uniquement colonnes numériques)
    """
    df = df.copy()
    ...
    # Convertir la cible en numérique si besoin
    ...
    # sécurité : s'assurer que la cible est dedans
    if target_col not in numeric_df.columns:
        numeric_df[target_col] = df[target_col]

    # corrélations
    corr_with_target = numeric_df.corr(numeric_only=True)[target_col].sort_values(ascending=False)
    top_corr = corr_with_target.drop(labels=[target_col], errors="ignore").head(20)

    if top_corr.empty:
        logger.warning("Aucune feature numérique pour calculer des corrélations.")
        return
